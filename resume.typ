...
#activityList(
  ...
  (
    ...
    activityEntry(
      ...
      - 자료 및 사용되는 도커파일에서 버전 호환성으로 인한 버그 추적 및 버전 고정
...
이러한 여러 소통 과정에서는 다양한 사람이 참여합니다. PO, 디자이너, 백엔드 개발자가 프론트엔드 개발자와 함께 제품의 기획에 관해 이야기하기도 하며, 제품의 디자인적 요소가 어떻게 사용자에게 영향을 끼칠지, 이를 파악하기 위해 어떤 방법을 사용할 것, 필요한 데이터는 무엇이며 어떤 형태로 주고받아야 하는지 논의하기도 합니다. 이러한 과정에서 제일 중요한 점은 자신을 포함한 팀원의 의견과 의도를 알고 반영하는 것입니다. 서로 다른 의견이 나오는 경우에도, 상대방의 의견과 의도, 맥락을 정확히 파악하면 더 좋은 하나의 답을 찾아 나설 수 있었습니다.

짧게는 해커톤에서부터, 길게는 연 단위 프로젝트에서도 마찬가지였습니다. 저는 꾸준히 팀원과 제 의도와 의견을 기록하는 일 또한 해왔으며, 이를 통해 프론트엔드 엔지니어라는 본래의 역할에서도 그 의견과 의도를 반영한 코드를 작성할 수 있었습니다. 이러한 과정을 즐기면서 개발을 해왔기에, PO, 디자이너, 백엔드, 인프라 등 여러 분야의 팀원들과 소통하며 개발하는 프론트엔드 개발자를 지향하게 되었습니다.
...
제 고향 여수는 관광지로 인해 형성된 상권이 많고, 이에 따라 여러 점주들이 들어왔다가 사라지는 모습을 보입니다. 저희 부모님처럼 오랫동안 한 분야에서 장사를 하면서 나름의 물류 관리 체계, 손님 관리 노하우, 제품 관리 체계를 지니신 분들도 있었지만, 모든 사람이 그런 것은 아니었습니다. 많은 업종들이 다양한 이유로 들어왔다가 사라져 갑니다. 그렇지만 살아남은 가게들은 적어도 손님들에게 가게의 사용 경험 과정에서 만족감을 준다는 공통점이 있었습니다. 결제 과정에서 버벅이지 않고, 좋은 상품을 제공하고, 가게에서의 경험이 불쾌한 경험이 되지 않도록 편안한 시스템을 제공했습니다.

토스플레이스는 이러한 경험을 보편화 할 수 있도록 사업을 합니다. B2B 대상인 점주를 넘어, 점주의 대상은 고객을 만족시키는 사업을 합니다. 이를 위해 토스플레이스는 키오스크 모드, 배달 플랫폼 작업 일원화를 통해서 점주의 역량을 집중할 수 잇도록 돕고, 고객에게는 안정적인 결제와 다양한 플랫품을 통한 결제 수단을 제공해서 만족스러운 경험을 제공합니다. 앞에서 말한 성공적인 가게에서 찾을 수 있는 그런 요소들입니다.

또한 토스플레이스는 점주와 고객을 잇는 오프라인 플랫폼입니다. 그리고 다른 토스의 사업에 비해서 초기 사업에 속해 있습니다. 그래서 변수가 많을 수 밖에 없고, 그런 변수를 검증하기 위하 카나리 릴리즈 같은 방식도 사용하고 있습니다. 이런 환경이면 의견과 의도 교환이 더 자주 일어나기 마련입니다. 이런 환경에서 그 의견과 의도를 코드에 반영하는 것, 그것이 프론트엔드 개발자로서 제 목표입니다. 그렇기에 저는 토스플레이스에 관심을 가지고 지원하게 되었습니다.
...
이러한 경험이 반복되면서 실패를 두려워하지 않게 되었습니다. 제가 특별히 두려움이 없는 사람이어서가 아니라, 적절한 기록을 남긴다면 실패를 통해서도 배울 수 있고, 혼자서는 해결 못 하는 실패더라도 같은 분야의 사람들, 다른 분야의 사람들에게 도움을 받을 수 있다는 사실을 배웠기 때문입니다.

첫 번째 프로젝트는 서비스 중이었던 장고 기반의 교내 커뮤니티 사이트, 지스토리의 관리와 개발이었습니다. 이 프로젝트의 문제점은 마스터로 머지하면 이전에 해결한 줄 알았던 버그들이 다시 생기는 것이었습니다. 그래서 해결했던 버그를 다시 해결하기도 했고, 같은 버그를 해결하기 위한 서로 다른 방법들이 꼬여서 다른 버그를 만들기도 했습니다. 문제의 원인은 잘못된 Git 정책이었습니다. Git을 각 작업을 기록하는 커밋의 용도로만 사용했을 뿐, 브랜치 별로 작업을 나눠서 관리한다는 개념이 제대로 반영되어 있지 않았습니다. 이미 작업이 끝나서 마스터로 머지한 브랜치를 여러 번 재사용하는 경우도 있었습니다.

이후 프로젝트부터는 일괄적으로 GitHub-flow에 기반한 브랜치 관리 전략을 일괄적으로 도입했습니다. 또한 이슈에 따라 브랜치와 일을 분배하기 시작하면서 이전의 문제가 해결되었습니다. 일반적인 GitHub-flow와 차이점은 메인과 구별되는 디벨롭 브랜치의 운영이었습니다. 디벨롭 브랜치의 용도는 머지 했을 때 문제점이 없는지 확인하는 용도로, 메인과 별도로 배포를 진행했습니다. 여기서 문제가 없는지 확인한 다음에 메인에 머지하여 배포를 진행했습니다. 이를 통해 배포 시의 문제점을 더욱 명확하게 파악할 수 있었고, 다른 프로젝트에서 타입스크립트 마이그레이션 같은 대규모 작업을 시도할 수 있는 계기가 되었습니다.

두 번째 프로젝트는 지스토리를 리액트 기반으로 새로 만드는 것이었습니다. 이 프로젝트에서부터 본격적으로 프론트엔드 엔지니어의 역할을 맡게 되었습니다. 리액트 스터디와 동시에 진행되었던 이 프로젝트는 개발이 길어지면서 학교와 개인의 일정에 따라 프로젝트에 참여하는 인원이 변동되게 되었습니다. 이런 이동이 반복되면서 각자 자신이 맡은 부분만 제대로 이해하거나, 다른 사람이 맡은 코드에 대해 이해하지 못해서 추후 작업 분배에 악영향을 끼치게 되었습니다.

이런 상황이 지속되는 것을 막기 위해, PR 시 진행되는 코드 리뷰랑 별개로 코드 리뷰를 진행하기로 했습니다. 이 코드 리뷰 시간을 통해 팀원끼리 코드의 작동에 대한 이해도를 비슷한 수준으로 맞출 수 있었습니다. 그리고 또 다른 방법으로, 그전까지는 메신저로만 기록되던 회의 기록을 노션과 같은 별도 형태로도 남기어서 다음에 작업에 참여하는 사람들이 프로젝트의 의도를 이해할 수 있도록 도왔습니다.

첫 번째 프로젝트의 Git 관리 문제를 해결하고, 두 번째 프로젝트의 인사 관리 문제를 해결하면서, 보다 더 커다란 기술적 문제 해결에 도전할 수 있게 되었습니다. 당시 컴포넌트 갯수가 늘어남에 따라 프롭 관리에 어려움이 늘어났고, 대부분의 버그는 프롭으로 넘겨주는 데이터의 타입이 잘못되어서 일어나는 일이었습니다. 특히 로딩 상태 관리를 `undefined | boolean` 로 관리했기 때문에 이러한 오류를 찾기가 더 힘들었습니다. 이 오류를 해결하기 위해 타입스크립트 마이그레이션을 주도하여 진행하였습니다. 앞에서 진행한 정기적인 리뷰 타임과 브랜치 전략 덕분에 장기 프로젝트임에도 큰 문제 없이 진행할 수 있었습니다. 그리고 이를 통해 타입이 불일치하는 부분을 찾아서 여러 오류들을 해결할 수 있게 되었고, 서버에서 전달해주는 데이터 스키마를 명확하게 공유할 수 있게 되었습니다.

이러한 기술적인 문제 조차도 해결했지만, 이용자 수가 적은 문제를 해결하지는 못했습니다. 애초에 목표 사용자들이 원하던 형태가 BBS와는 거리가 멀었기 때문이었습니다. 추후에 합류한 기획 쪽 팀원의 이러한 의견을 인정하고 실패를 받아들였습니다. 그리고 그 팀원의 도움을 받아 실패 원인과 목표 사용자를 더 집중적으로 정리할 수 있었습니다. 이 기획 자료를 전달 받은 후배 팀원들은 공지사항 전달에 집중한 형태의 서비스를 만들어서 더 많은 사용자를 모을 수 있었습니다.

인포팀이 아닌 프로젝트에서도 비슷하게 실패를 겪으면 거기서 멈추는 것이 아니라 바로 그 실패의 교훈을 정리하였습니다. 실험조교로 실험 수업을 진행할 때에도 매 주 첫 수업 이후에 실험 과정 중에 발생한 에러들을 정리해서 해결책과 기록을 남겨 놓았고, 이를 통해 그 이후 수업과 내후년도 수업에서는 그러한 에러들이 발생하지 않도록 할 수 있었습니다.
