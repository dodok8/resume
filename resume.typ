...
짧은 프로젝트에서도, 긴 프로젝트에서도 저는 프론트엔드 엔지니어라는 역할 외에도 저는 꾸준히 팀원과 제 의도와 의견을 기록하는 일 또한 해왔습니다. 해커톤에서는 토론이 길을 잃고 길어질 때마다 각자가 이때까지 한 발언을 정리해서 올리는 것으로 의도와 의견을 모아서 정리 하였습니다. 장기 개발 프로젝트에서는 정기적인 코드리뷰 시간을 운영하고 팀원들간의 논의와 결과를 회의록에 정리했습니다.

 이를 통해 프론트엔드 엔지니어라는 본래의 역할에서도 그 의견과 의도를 반영한 코드를 작성할 수 있었습니다. 이러한 과정을 즐기면서 개발을 해왔기에, PO, 디자이너, 백엔드, 인프라 등 여러 분야의 팀원들과 소통하며 개발하는 프론트엔드 개발자를 지향하게 되었습니다.
...
첫 번째 프로젝트의 Git 관리 문제를 해결하고, 두 번째 프로젝트의 인사 관리 문제를 해결하면서, 보다 더 커다란 기술적 문제 해결에 도전할 수 있게 되었습니다. 당시 컴포넌트 갯수가 늘어남에 따라 프롭 관리에 어려움이 늘어났고, 대부분의 버그는 프롭으로 넘겨주는 데이터의 타입이 잘못되어서 일어나는 일이었습니다. 특히 로딩 상태 관리를 `undefined | boolean` 로 관리했기 때문에 이러한 오류를 찾기가 더 힘들었습니다. 이 오류를 해결하기 위해 타입스크립트 마이그레이션을 주도하여 진행하였습니다. 정기적인 리뷰 타임과 브랜치 전략 덕분에 큰 문제없이 진행할 수 있었고, 이를 통해 타입이 불일치하는 부분을 찾아서 관련 오류를 해결할 있었으며, 서버와 데이터 스키마를 명확하게 공유할 수 있게 되었습니다.

이렇게 기술적인 문제 조차도 해결했지만, 이용자 수가 적은 문제를 해결하지는 못했습니다. 애초에 목표 사용자들이 원하던 형태가 BBS와는 거리가 멀었기 때문이었습니다. 추후에 합류한 기획 쪽 팀원의 이러한 의견을 인정하고 실패를 받아들였습니다. 그리고 그 팀원의 도움을 받아 실패 원인과 목표 사용자를 더 집중적으로 정리할 수 있었습니다. 이 기획 자료를 전달 받은 후배 팀원들은 공지사항 전달에 집중한 형태의 서비스를 만들어서 더 많은 사용자를 모을 수 있었습니다.
