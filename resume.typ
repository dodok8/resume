...
또한 토스플레이스는 점주와 고객을 잇는 오프라인 플랫폼입니다. 그리고 다른 토스의 사업에 비해서 초기 사업에 속해 있습니다. 그래서 그에 따른 변수가 많은 수 밖에 없고, 그런 변수를 검증하기 위하 카나리 릴리즈 같은 방식도 사용하고 있습니다. 이런 환경이면 더 많은 의견과 의도 교환이 더 자주 일어나기 마련입니다. 이런 과정에서 그 의견과 의도를 코드에 반영하는 것, 그것이 프론트엔드 개발자로서 제 목표입니다. 그렇기에 저는 토스플레이스에 관심을 가지고 지원하게 되었습니다.
...
두 번째 프로젝트는 지스토리를 리액트 기반으로 새로 만드는 것이었습니다. 이 프로젝트에서부터 본격적으로 프론트엔드 엔지니어의 역할을 맡게 되었습니다. 리액트 스터디와 동시에 진행되었던 이 프로젝트는 개발이 길어지면서 학교와 개인의 일정에 따라 프로젝트에 참여하는 인원이 변동되게 되었습니다. 이런 이동이 반복되면서 각자 자신이 맡은 부분만 제대로 이해하거나, 다른 사람이 맡은 코드에 대해 이해를 하지 못해서 추후 작업 분배에 악영향을 끼치게 되었습니다.

이런 상황이 지속되는 것을 막기 위해, PR 시 진행되는 코드 리뷰랑 별개로 코드 리뷰를 진행하기로 했습니다. 이 코드 리뷰 시간을 통해 팀원들 끼리 코드의 작동에 대한 이해도를 비슷한 수준으로 맞출 수 있었습니다. 그리고 또 다른 방법으로, 그 전까지는 메신저로만 기록되던 회의 기록을 노션과 같은 별도 형태로도 남기어서 추후에 작업에 참여하는 사람들이 프로젝트의 의도를 이해할 수 있도록 도왔습니다.
